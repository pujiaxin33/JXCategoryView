...
- (void)contentOffsetOfContentScrollViewDidChanged:(CGPoint)contentOffset {
    ...
    CGFloat targetindicatorLineWidth = self.indicatorLineWidth;
    ...
    if (self.lineStyle == JXCategoryLineStyle_JD) {
        if (remainderRatio == 0) {
            ...
            CGFloat rightXOfIndicatorLineView = rightCellFrame.origin.x + (rightCellFrame.size.width - self.indicatorLineWidth)/2;
            CGFloat minWidth = targetindicatorLineWidth;
            CGFloat maxWidth = rightXOfIndicatorLineView - leftXOfIndicatorLineView + self.indicatorLineWidth;
            //前50%，只增加width；后50%，移动x并减小width
            if (remainderRatio <= 0.5) {
                totalXOfIndicatorLineView = leftXOfIndicatorLineView;
                targetindicatorLineWidth = [self interpolationFrom:minWidth to:maxWidth percent:remainderRatio*2];
            }else {
                ...
                targetindicatorLineWidth = [self interpolationFrom:maxWidth to:minWidth percent:(remainderRatio - 0.5)*2];
            }
        }
        ...
        if (remainderRatio == 0) {
            ...
            CGFloat minWidth = targetindicatorLineWidth;
            CGFloat maxWidth = rightXOfIndicatorLineView - leftXOfIndicatorLineView + self.indicatorLineWidth - offsetX*2;
            if (remainderRatio <= 0.5) {
